"""Build the selected-works site: index.html (layout, no text) + content.js (ALL text + works).
Usage: python build_site.py            -> works/          (full copy)
       BARE=1 python build_site.py     -> works/bare/     (tagline only)
"""
import os, json, shutil, re
S = os.path.dirname(os.path.abspath(__file__))
exec(open(os.path.join(S, "build_final.py"), encoding="utf-8").read().split("CSS = ")[0])  # works, descs, COPY, P
...
content = {
  ...
  "statement_tagline": "I apply the machine as a means to express form." if BARE else COPY["lede"],
  "statement": [] if BARE else COPY["brief"],
  "about_tagline": "" if BARE else COPY["about_lede"],
  "about": [] if BARE else COPY["about"],
  "process_tagline": "" if BARE else COPY["process_lede"],
...
<title>Evan Emery Selected Works</title>
<meta name="description" content="{content['statement_tagline']} Nine works, 2024 to 2025, in cyanotype, plotted ink and pigment.">
<meta property="og:type" content="website"><meta property="og:title" content="Evan Emery · Selected Works">
<meta property="og:description" content="{content['statement_tagline']} Nine works, 2024 to 2025.">
<meta property="og:image" content="https://evanemerydesign.github.io/works/img/deep-blue.jpg">
<meta property="og:url" content="https://evanemerydesign.github.io/works/{'bare/' if BARE else ''}">
...
.nav ul {{ list-style:none; margin:0; padding:0; display:flex; gap:14px 22px; flex-wrap:wrap; justify-content:flex-end; }}
...
.hero .shade {{ position:absolute; inset:0; background:linear-gradient(to top, rgba(8,14,30,.7) 0%, rgba(8,14,30,.25) 45%, rgba(8,14,30,0) 75%); }}
.hero .in {{ position:absolute; left:0; right:0; bottom:0; max-width:1400px; margin:0 auto; padding:0 24px 32px; display:flex; justify-content:space-between; align-items:flex-end; gap:16px; flex-wrap:wrap; }}
.hero .t, .hero .r {{ background:rgba(8,14,30,.88); padding:22px 26px; }}
.hero h1 {{ margin:0; font-size:clamp(34px, 6vw, 72px); line-height:.94; letter-spacing:-.01em; text-wrap:balance; }}
.hero .sub {{ margin-top:14px; font-family:'Space Mono', monospace; font-size:11px; letter-spacing:.26em; text-transform:uppercase; }}
.hero .r {{ font-family:'Space Mono', monospace; font-size:11px; line-height:1.9; letter-spacing:.06em; text-align:right; }}
...
.brief .in.solo {{ grid-template-columns:1fr; }}
@media (max-width:900px) {{ .brief .in {{ grid-template-columns:1fr; gap:20px; }} }}
.lede {{ margin:0; font-family:'Chakra Petch', 'Arial Narrow', sans-serif; font-weight:700; font-size:clamp(26px, 3.2vw, 40px); line-height:1.05; text-transform:uppercase; text-wrap:balance; }}
.solo .lede {{ font-size:clamp(30px, 4.2vw, 56px); max-width:22ch; }}
.body p {{ margin:0 0 14px; font-size:17px; line-height:1.55; max-width:62ch; }}
...
.card.fill .box {{ padding:0; overflow:hidden; }}
.card.fill img {{ width:100%; height:100%; object-fit:cover; box-shadow:none; }}
...
.lb .bg {{ position:absolute; inset:0; pointer-events:none; background-image:linear-gradient(to right, rgba(0,0,0,.055) 1px, transparent 1px), linear-gradient(to bottom, rgba(0,0,0,.055) 1px, transparent 1px), linear-gradient(to right, rgba(0,0,0,.028) 1px, transparent 1px), linear-gradient(to bottom, rgba(0,0,0,.028) 1px, transparent 1px); background-size:48px 48px, 48px 48px, 12px 12px, 12px 12px; }}
...
.ptxt {{ max-width:60ch; }}
.ptxt h3 {{ margin:0 0 14px; font-size:clamp(24px, 2.6vw, 34px); line-height:1.05; text-transform:uppercase; letter-spacing:.02em; }}
...
<header class="hero">
  ...
  <div class="in">
    <div class="t"><h1 class="cp"><span data-t="name"></span><br><span data-t="title"></span></h1><div class="sub" data-t="subtitle"></div></div>
    <div class="r"><span data-t="location"></span><br><a id="heroIg" href="#" data-t="instagram"></a></div>
  </div>
</header>

<section class="brief" id="statement"><div class="in" id="briefIn">
  <div><p class="lede" data-t="statement_tagline"></p><div class="lab">Artist statement</div></div>
...
<section class="s" id="process" style="background:var(--blue2); color:#f4f0ea"><div class="wrap">
  ...
  <div class="proc">
    <div class="vid"><video id="btsVideo" src="{rel}bts.mp4" poster="{rel}img/bts-poster.jpg" controls playsinline preload="metadata"></video></div>
    <div class="ptxt"><h3 class="cp" id="processTag"></h3><div id="processBody"></div><p class="lab" style="color:#b9bec9; margin-top:18px" data-t="process_note"></p></div>
  </div>
</div></section>
...
<section class="s" id="about" style="background:var(--bg2)"><div class="wrap">
  ...
  <div class="about"><p class="lede" data-t="about_tagline"></p><div class="body" id="aboutBody"></div></div>
</div></section>

<section class="s" id="list"><div class="wrap">
  <div class="hd"><h2 class="cp">Available works</h2><span class="lab" data-t="list_label"></span></div>
  <div class="two">
    <div class="tw"><table><thead><tr><th>#</th><th>Title</th><th>Year</th><th>Medium</th><th>Dimensions</th><th>Retail</th></tr></thead><tbody id="rows"></tbody></table></div>
    <div class="contact"><h3 class="cp">Contact</h3>
      <div><span class="k lab">Name</span><span>Evan Emery</span></div>
      <div><span class="k lab">Location</span><span data-t="location"></span></div>
      <div><span class="k lab">Email</span><a id="mail" href="#" data-t="email"></a></div>
      <div><span class="k lab">Instagram</span><a id="ig" href="#" data-t="instagram"></a></div>
      <div><span class="k lab">LinkedIn</span><a id="li" href="#" data-t="linkedin"></a></div>
    </div>
  </div>
</div></section>

<footer><div class="in"><span class="lab" data-t="footer_left"></span><span class="lab" data-t="footer_right"></span></div></footer>
...
document.title = 'Evan Emery ' + (C.title || '').replace(/\\b\\w+/g, w => w[0] + w.slice(1).toLowerCase());
$('heroIg').href = $('ig').href = C.instagram_url; $('mail').href = 'mailto:' + C.email; $('li').href = C.linkedin_url;
$('statementBody').innerHTML = paras(C.statement); if (!C.statement || !C.statement.length) $('briefIn').classList.add('solo');
$('processTag').textContent = C.process_tagline || ''; $('processBody').innerHTML = paras(C.process);
...
if not BARE:
    ...
    for w in works:
        shutil.copy(os.path.join(S, "img", w[0] + ".jpg"), os.path.join(OUT, "img", w[0] + ".jpg"))
print("built", "bare" if BARE else "full", "->", OUT)
